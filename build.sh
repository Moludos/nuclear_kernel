...
NUCLEAR_VERSION="V.7-toub6-CM12.1"
EXTENDED_CMDLINE="androidboot.selinux=permissive"

TOOLCHAIN="/Android/toolchains/uber6/bin/arm-eabi-"
...
NUCLEAR_FILENAME="nuclear-kernel-$NUCLEAR_VERSION"
...
step0_copy_code()
{
	...
	sed "s/\`echo \$LINUX_COMPILE_BY | \$UTS_TRUNCATE\`/Nuclear-Kernel-$NUCLEAR_VERSION-$NUCLEAR_DATE/g" -i $BUILD_PATH/scripts/mkcompile_h
	sed "s/\`echo \$LINUX_COMPILE_HOST | \$UTS_TRUNCATE\`/moludo" -i $BUILD_PATH/scripts/mkcompile_h
}
...
step2_make_config()
{
	...
	echo
	export ARCH=arm
        export CROSS_COMPILE=~/Android/toolchains/uber6/bin/arm-eabi-
...
step3_compile()
{
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		...
		chmod 777 tools_nuclear/dtbToolCM
		tools_nuclear/dtbToolCM -2 -o $BUILD_PATH/$OUTPUT_FOLDER/arch/arm/boot/dt.img -s 2048 -p $BUILD_PATH/$OUTPUT_FOLDER/scripts/dtc/ $BUILD_PATH/$OUTPUT_FOLDER/arch/arm/boot/
	fi
...
step5_patch_ramdisk()
{
	...
	# Apply nuclear kernel specific patches and copy additional files to ramdisk
	echo -e ">>> apply Nuclear kernel patches and copy files\n"

	cd $REPACK_PATH
	for PATCHFILE in $BUILD_PATH/ramdisk_nuclear/patch/*.patch
	do
		...
		
	{
		...
		cp -R $BUILD_PATH/ramdisk_nuclear/fs/* ramdisk
...
step6_repack_ramdisk()
{
	...
	cd $REPACK_PATH
	chmod 777 $BUILD_PATH/tools_nuclear/mkbootimg
	
	if [ "y" == "$COMPILE_DTB" ]; then
		$BUILD_PATH/tools_nuclear/mkbootimg --kernel zImage --ramdisk newramdisk.cpio.gz --cmdline "$MKBOOTIMG_CMDLINE $EXTENDED_CMDLINE" --base $MKBOOTIMG_BASE --pagesize $MKBOOTIMG_PAGESIZE --ramdisk_offset $MKBOOTIMG_RAMDISK_OFFSET --tags_offset $MKBOOTIMG_TAGS_OFFSET --dt dt.img -o boot.img
	else
		$BUILD_PATH/tools_nuclear/mkbootimg --kernel zImage --ramdisk newramdisk.cpio.gz --cmdline "$MKBOOTIMG_CMDLINE $EXTENDED_CMDLINE" --base $MKBOOTIMG_BASE --pagesize $MKBOOTIMG_PAGESIZE --ramdisk_offset $MKBOOTIMG_RAMDISK_OFFSET --tags_offset $MKBOOTIMG_TAGS_OFFSET -o boot.img
	fi
	...
	cp $BUILD_PATH/tools_nuclear/update-binary META-INF/com/google/android
	...
	echo "ui_print(\"Flashing Nuclear-Kernel $NUCLEAR_VERSION\");" >> META-INF/com/google/android/updater-script
	...
	echo "ui_print(\"By ...Moludo, $(date +%Y.%m.%d-%H:%M:%S)\");" >> META-INF/com/google/android/updater-script
	echo "ui_print(\" \");" >> META-INF/com/google/android/updater-script
	echo "ui_print(\"Finished, please reboot.\");" >> META-INF/com/google/android/updater-script

	# add required files to new zip
	zip $NUCLEAR_FILENAME.recovery.zip boot.img
	zip $NUCLEAR_FILENAME.recovery.zip META-INF/com/google/android/updater-script
	zip $NUCLEAR_FILENAME.recovery.zip META-INF/com/google/android/update-binary

	if [ ! "y" == "$KERNEL_SAMSUNG" ]; then
		zip $NUCLEAR_FILENAME.recovery.zip $MODULE_PATH/*
	fi

	# sign recovery zip if there are keys available
	if [ -f "$BUILD_PATH/tools_nuclear/testkey.x509.pem" ]; then
		echo -e ">>> signing recovery zip\n"
		java -jar $BUILD_PATH/tools_nuclear/signapk.jar -w $BUILD_PATH/tools_nuclear/testkey.x509.pem $BUILD_PATH/tools_nuclear/testkey.pk8 $NUCLEAR_FILENAME.recovery.zip $NUCLEAR_FILENAME.recovery.zip_signed
		cp $NUCLEAR_FILENAME.recovery.zip_signed $NUCLEAR_FILENAME.recovery.zip
		rm $NUCLEAR_FILENAME.recovery.zip_signed
	fi

	md5sum $NUCLEAR_FILENAME.recovery.zip > $NUCLEAR_FILENAME.recovery.zip.md5

	# For Samsung kernels, create tar.md5 for odin
	if [ "y" == "$KERNEL_SAMSUNG" ]; then
		echo -e ">>> create Samsung files for Odin\n"
		cd $REPACK_PATH
		tar -cvf $NUCLEAR_FILENAME.tar boot.img
		md5sum $NUCLEAR_FILENAME.tar >> $NUCLEAR_FILENAME.tar
		mv $NUCLEAR_FILENAME.tar $NUCLEAR_FILENAME.tar.md5
	fi
	...
	if [ "y" == "$KERNEL_SAMSUNG" ]; then
		...
		cp $NUCLEAR_FILENAME.recovery.zip cm-kernel.zip
...
step8_transfer_kernel()
{
	...
	smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "mkdir $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION"
	smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$NUCLEAR_FILENAME.recovery.zip $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\$NUCLEAR_FILENAME.recovery.zip"
	smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$NUCLEAR_FILENAME.recovery.zip.md5 $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\$NUCLEAR_FILENAME.recovery.zip.md5"
	smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/checksum $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\checksum"

	if [ "y" == "$KERNEL_SAMSUNG" ]; then
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/$NUCLEAR_FILENAME.tar.md5 $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\$NUCLEAR_FILENAME.tar.md5"
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/boot.img $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\boot.img"
	else
		smbclient $SMB_SHARE_KERNEL -U $SMB_AUTH_KERNEL -c "put $REPACK_PATH/cm-kernel.zip $SMB_FOLDER_KERNEL\\$NUCLEAR_VERSION\\cm-kernel.zip"
	fi
}
...
step9_send_finished_mail()
{
	...
	if [ -z "$FINISH_MAIL_TO" ]; then
		...
		cat $ROOT_PATH/compile.log | /usr/bin/mailx -s "Compilation for Nuclear-Kernel $NUCLEAR_VERSION finished!!!" $FINISH_MAIL_TO
	fi
}	
...
echo "Parameters:"
echo
echo "  Nuclear version:  $NUCLEAR_VERSION"
echo "  Extended cmdline: $EXTENDED_CMDLINE"
echo "  Nuclear date:     $NUCLEAR_DATE"
