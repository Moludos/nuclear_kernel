# Nuclear-Config controller interface
#
# *****************************
...
if [ "lov_system_tweaks" == "$1" ]; then
	echo "Off;Nuclear tweaks;Speedmod tweaks;Mattiadj tweaks"
	exit 0
fi
...
#	echo "1 4027 1031 0 276" > /sys/class/misc/nuclear_sound/eq_bands
#	echo "2 8076 61555 456 456" > /sys/class/misc/nuclear_sound/eq_bands
#	echo "3 7256 62323 2644 1368" > /sys/class/misc/nuclear_sound/eq_bands
#	echo "4 5774 63529 1965 4355" > /sys/class/misc/nuclear_sound/eq_bands
#	echo "5 1380 1369 0 16384" > /sys/class/misc/nuclear_sound/eq_bands
	exit 0
fi
...
if [ "apply_survival_script" == "$1" ]; then
	if [ "1" == "$2" ]; then
		...
		busybox cp /res/misc/97-nuclear-kernel.sh /system/addon.d
		busybox chmod 755 /system/addon.d/97-nuclear-kernel.sh
		busybox sync
	...
	if [ "0" == "$2" ]; then
		mount -o remount,rw -t ext4 $SYSTEM_DEVICE /system
		busybox rm /system/addon.d/97-nuclear-kernel.sh
		busybox sync
		mount -o remount,ro -t ext4 $SYSTEM_DEVICE /system
	fi
	exit 0
...
if [ "action_debug_info_file" == "$1" ]; then
	...
	echo -e "\n**** Nuclear-Kernel log\n" >> $2
	cat /sdcard/nuclear-kernel-data/nuclear-kernel.log >> $2

	echo -e "\n**** Nuclear-Kernel log 1\n" >> $2
	cat /sdcard/nuclear-kernel-data/nuclear-kernel.log.1 >> $2

	echo -e "\n**** Nuclear-Kernel log 2\n" >> $2
	cat /sdcard/nuclear-kernel-data/nuclear-kernel.log.2 >> $2

	echo -e "\n**** Nuclear-Kernel log 3\n" >> $2
	cat /sdcard/nuclear-kernel-data/nuclear-kernel.log.3 >> $2

	echo -e "\n============================================\n" >> $2

	echo -e "\n**** Nuclear-Config app log\n" >> $2
	cat /sdcard/nuclear-kernel-data/bc.log >> $2

	echo -e "\n**** Nuclear-Config app log 1\n" >> $2
	cat /sdcard/nuclear-kernel-data/bc.log.1 >> $2

	echo -e "\n**** Nuclear-Config app log 2\n" >> $2
	cat /sdcard/nuclear-kernel-data/bc.log.2 >> $2

	echo -e "\n**** Nuclear-Config app log 3\n" >> $2
	cat /sdcard/nuclear-kernel-data/bc.log.3 >> $2

	echo -e "\n**** Nuclear-Config crash log\n" >> $2
	cat /sdcard/nuclear-kernel-data/bc.crashlog >> $2

	echo -e "\n============================================\n" >> $2

	echo -e "\n**** nuclear_sound\n" >> $2
	cd /sys/class/misc/nuclear_sound
